...
func Worker(mapf func(string, string) []KeyValue,
	...
	alive := true
	for alive {
		fmt.Println("worker", os.Getegid(), "请求任务")
		task := RequestTask()
		switch task.TaskType {
		case MAP:
			{
				fmt.Println("task id: ", task.Id, "MAP 任务处理中")
				doMapTask(mapf, task)
				fmt.Println("task id: ", task.Id, "MAP 任务完成")
				doneTask(task, MAPPED)
			}
		case REDUCE:
			{
				fmt.Println("task id: ", task.Id, "REDUCE 任务处理中")
				doReduceTask(reducef, task)
				fmt.Println("task id: ", task.Id, "REDUCE 任务完成")
				doneTask(task, REDUCED)
			}
		case END:
			{
				alive = false
				fmt.Println("worker任务已完成...")
			}
		}
	}
}
...
func doReduceTask(reducef func(string, []string) string, task TaskInfo) {
	reduceFileNum := task.Id
	intermediate := readFromLocalFile(task.ProcessFile)
